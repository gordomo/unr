...
function verPass() {
    if ($("#password").prop("type") == "password") {
        ...
        pass.type = 'text';
        pass.value = original.val();
        pass.name = original.attr("name");
        pass.id = original.attr("id");
        $(pass).addClass(original.attr("class"));
        pass.placeholder = original.attr("placeholder");
        pass.required = original.attr("required");
        original.parent().append(pass);
        original.remove();
        $("#verPass").html("ocultar contraseña");
    } else {
        var original = $("#password"); 
        var pass = document.createElement('input');
        pass.type = 'password';
        ...
        original.parent().append(pass);
        original.remove();
        $("#verPass").html("ver contraseña");
    }

}

$(".editar").on("click", function(){
    var idInput = $(this).data("cat-id") + "-edited";
    var idCat = $(this).data("cat-id");
    var val = $("#"+idInput).val();

    if($(this).data("do") == "e") {
        var categoriaAeditar = document.createElement("input");
        var id = $(this).data("cat-id");
        var td = $("#"+id);
        var txt = td.html();
        td.html('');
        categoriaAeditar.value = $.trim(txt);
        categoriaAeditar.id = id + "-edited";
        td.append(categoriaAeditar);
...
$(".editar-apunte").click(function(){
    ...
    if($(this).data("do") == "e") {
        $("#" + id_form + " .ab").attr("disabled", false);
        $(this).html('<i class="fa fa-check" aria-hidden="true"></i>');
        $(this).data("do", "s");
    } else {
        var id = $(this).data("apunte-id");
...
$(".borrar-apunte").on("click", function(){
    var id = $(this).data("apunte-id");
    location.href = "controllers/apuntes_controller.php?action=borrarApunte&id="+id;
});


$(".borrarSub").on("click", function(){
    var id = $(this).data("cat-id");
...
$(".selectCategorias").change(function(){
    ...
    .done(function( msg ) {
        ...
        $(msg).each(function(){
            $('#sub-categoria-'+apunteId).append('<option value="'+this.id+'">'+this.name+'</option>');
        });
    });
...
$("#reenviarMailDeConfirmacion").click(function() {
    $("#reenviarMailDeConfirmacion").attr("disabled", true);
    var mail = $(this).data("mail");
    
    $.ajax({
      method: "POST",
      url: "../includes/process_login.php",
      data: { mail: mail, action: "reenviarMailDeConfirmacion" },
      dataType: "json"
    })
    .done(function( msg ) {
